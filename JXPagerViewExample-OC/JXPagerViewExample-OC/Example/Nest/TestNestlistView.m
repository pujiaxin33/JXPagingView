...
@interface TestNestlistView () <JXCategoryViewDelegate>
...
@implementation TestNestlistView
...
- (void)layoutSubviews {
    ...
    self.hobbyListView.frame = CGRectMake(self.bounds.size.width, 0, self.bounds.size.width, self.contentScrollView.bounds.size.height);
    self.partnerListView.frame = CGRectMake(self.bounds.size.width*2, 0, self.bounds.size.width, self.contentScrollView.bounds.size.height);
}
